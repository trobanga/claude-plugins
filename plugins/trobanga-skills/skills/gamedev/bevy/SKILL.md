...
### ⚠️ Bevy 0.18 Key API Changes

**Bevy 0.18** is the current target. Key patterns to know:
- Materials wrapped in `MeshMaterial3d<T>` (not `Handle<T>`)
- Events use the observer pattern (`commands.trigger()`, `add_observer()`, `Trigger<T>`)
- `EventReader`/`EventWriter` renamed to `MessageReader`/`MessageWriter`
- For mouse input, prefer `Res<AccumulatedMouseMotion>` and `Res<AccumulatedMouseScroll>` over manually iterating events
- `GltfAssetLabel::from_asset()` borrows its argument — can't use with dynamic `String` paths. Use `asset_server.load(format!("path/model.glb#Scene0"))` instead
- Color arithmetic requires component extraction (use `LinearRgba` or `.to_srgba()`)
- `BorderRadius` is a field on `Node`, not a separate component
- `AmbientLight` is per-camera; use `GlobalAmbientLight` for world-wide ambient
- `NextState::set()` always fires; use `set_if_neq()` to avoid redundant transitions
- Child methods renamed: `clear_children()` → `detach_all_children()`, `remove_children()` → `detach_children()`

**See `references/bevy_specific_tips.md` for complete API reference, 0.17→0.18 migration guide, and new features.**
...
impl Plugin for CombatPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, (check_death, update_health_bars));
    }
}
```
...
1. Observer pattern mistakes (forgetting `Clone` derive, infinite re-triggers)
2. Material handle access (`MeshMaterial3d<T>` wrapper, not raw `Handle<T>`)
3. Borrowing conflicts (use `get_many_mut` for multiple mutations)
4. Not using `Changed<T>` for expensive operations
5. Wrong system ordering (input → state → derived → visual → UI)
6. Entity queries after despawn (use `if let Ok()` pattern)
7. `BorderRadius` as separate component (it's a field on `Node` in 0.18)
8. Renamed child methods (`clear_children` → `detach_all_children`)
9. Redundant state transitions (`set()` always fires; use `set_if_neq()`)
10. `AmbientLight` scope (now per-camera; use `GlobalAmbientLight` for global)
11. glTF material patching: use `Added<MeshMaterial3d>` + `ChildOf` walk, not `SceneInstanceReady` observer
12. `Gltf::named_animations` HashMap: use exact key lookup, not `.contains()` search
13. `AnimationGraph` required even for single clips; `AnimationGraphHandle` goes on the `AnimationPlayer` descendant, not `SceneRoot`
14. **GGRS rollback destroys scene hierarchies**: Never put `SceneRoot`/animations and `.add_rollback()` on the same entity. Use the sim/visual entity split. See the **ggrs skill** for the full pattern.
15. **Root motion cancellation scheduling**: Must run in `PostUpdate` between `AnimationSystems` and `TransformSystems::Propagate`. Earlier (Update) is before animation evaluation; later (Last) is after GlobalTransform propagation. Target bone entities (`AnimationTargetId`), not the `AnimationPlayer`.
16. **AnimationTransitions**: Once inserted, never use `player.play()` directly — always use `transitions.play()`. Use `commands.queue()` with `EntityWorldMut` for setup-time calls when you can't borrow both mutably.

Review `references/common_pitfalls.md` before implementing complex features.

### Rollback Netcode (bevy_ggrs)

If the project uses bevy_ggrs, also invoke the **ggrs skill** (`~/.claude/skills/ggrs/`). Key integration concerns:
- Entities with `.add_rollback()` can be despawned during rollback — never attach scene hierarchies to them
- Deferred commands in observers are lost when GGRS despawns the target entity before flush
- Run-condition guards must use unregistered resources, not component queries on Rollback entities
- Game logic in `GgrsSchedule`, visual sync in `Update`
